        ...
        if instance.status == 'ACTIVE':
            notifications.extend(
                NotificationManager.create_notification(
                    instance,
                    'TRADE',
                    f"New trade activated: {instance.company.trading_symbol}",
                    f"A new trade has been activated for {instance.company.script_name}"
                )
            )
            ...
            notifications.extend(
                NotificationManager.create_notification(
                    ...
                    f"Trade for {instance.company.script_name} has been completed"
                )
            )
    ...
    if instance.status == 'ACTIVE' and instance.tracker.has_changed('image'):
        notifications.extend(
            NotificationManager.create_notification(
                instance,
                'TRADE',
                f"Chart updated: {instance.company.trading_symbol}",
    ...
    if instance.status == 'ACTIVE' and instance.tracker.has_changed('warzone'):
        notifications.extend(
            NotificationManager.create_notification(
                instance,
                'RISK',
                f"Risk level updated: {instance.company.trading_symbol}",
...
#     short_message = f"New trade alert for {trade.company}" if trade.status == "ACTIVE" else f"Trade completed: {trade.company}"
#     related_url = f"/trades/{trade.id}"
#     trade_content_type = ContentType.objects.get_for_model(Trade)
#     trade_data = {
#                     'company': trade.company.trading_symbol,
